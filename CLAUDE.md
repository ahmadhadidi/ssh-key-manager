...
| `ssh-ops.ps1` | ~570 | SSH key generation, deploy, install, test, remove, promote | `Add-SSHKeyInHost`:9, `Add-SSHKeyToHostConfig`:40, `Deploy-SSHKeyToRemote`:219, `Test-SSHConnection`:234 |
| `config-display.ps1` | ~450 | SSH config viewer, key inventory display, host removal | `Show-SSHConfigFile`:56, `Show-SSHKeyInventory`:172, `Remove-HostFromSSHConfig`:6 |
