...
| `menu-renderer.ps1` | ~220 | TUI event loop and operation runner | `Show-MainMenu`:6, `_InvokeMenuAction`:221 |
...
| `ssh-ops.ps1` | config-display, menu | `Add-SSHKeyInHost` (called from `Deploy-SSHKeyToRemote`); `Add-SSHKeyToHostConfig` (called from `Install-SSHKeyOnRemote`, `Register-RemoteHostConfig`, `Add-KeyToHosts`); all 12 public functions called from `_Menu*` handlers in menu.ps1 |
| `config-display.ps1` | menu only | `Add-SSHKeyInHost` (menu `_MenuGenerateKey`); `Add-SSHKeyToHostConfig` (ssh-ops×3, menu `_MenuAppendKeyToConfig`); `Show-SSHConfigFile`/`Edit-SSHConfigFile`/`Show-SSHKeyInventory`/`Remove-HostFromSSHConfig` (menu only) |
...
- `Resolve-SSHTarget`:8 `RemoteHostAddress RemoteUser` — returns `user@alias` if a config block exists (by alias or HostName match), so SSH applies the full block (including `ServerAliveInterval`, `ForwardAgent`, etc.); falls back to `user@IP`
- `Install-SSHKeyOnRemote`:36 `KeyName` — copies public key to remote `authorized_keys`, then prompts for alias and calls `Add-SSHKeyToHostConfig`
- `Register-RemoteHostConfig`:80 — connects to a not-yet-configured host, reads `authorized_keys`, matches against local `.pub` files, creates a config block
- `Deploy-SSHKeyToRemote`:138 `KeyName` — generates if missing (`Add-SSHKeyInHost`), then installs (`Install-SSHKeyOnRemote`)
- `Test-SSHConnection`:153 `RemoteUser RemoteHost [IdentityFile]` — TCP probe first (3 s, port 22); with `$IdentityFile` uses `-F NUL -i key -o IdentitiesOnly=yes -o PreferredAuthentications=publickey` to bypass config and isolate the key; **no `-o BatchMode=yes`** so passphrase prompts work; without identity uses `Resolve-SSHTarget`
- `Remove-IdentityFileFromConfigBlock`:210 `KeyName HostAlias` — removes all `IdentityFile` lines for `$KeyName` from the named Host block via regex replace
- `Remove-SSHKeyFromRemote`:234 `RemoteUser RemoteHost KeyName` — builds a remote `awk` command to filter `authorized_keys` in-place; offers to delete local key pair afterwards
- `Deploy-PromotedKey`:274 — key rotation: prompts old key + remote + new key; deploys new, then optionally removes old
- `Add-KeyToHosts`:294 `KeyName` — multi-select configured hosts via `Select-MultiFromList`; appends `IdentityFile` to each chosen block via `Add-SSHKeyToHostConfig`
- `Import-ExternalSSHKey`:329 — three modes: local path (`Copy-Item`), SCP from remote, or paste; all paths end with `Add-KeyToHosts`
- `Remove-IdentityFileFromConfigEntry`:434 `KeyName RemoteHostName` — similar to `Remove-IdentityFileFromConfigBlock` but matches by hostname; splits config on newlines rather than full regex replace
- `Invoke-SSHWithKeyThenPassword`:467 — key-first with `BatchMode=yes`; falls back to password prompt on `Permission denied`; not used by `Test-SSHConnection` (separate code paths)

### config-display.ps1

- `Add-SSHKeyInHost`:6 `KeyName Comment` — prompts passphrase via `Read-Host -AsSecureString` (masked); runs `ssh-keygen -t ed25519` via `Invoke-Expression`; key name and comment must not contain shell-special characters
- `Add-SSHKeyToHostConfig`:37 `KeyName RemoteHostName RemoteHostAddress RemoteUser` — if block exists: inserts `IdentityFile` after the last existing one (or after the `Host` line if none); skips if already present. If block absent: appends a new block at EOF.
- `Remove-HostFromSSHConfig`:86 — shows block preview before confirming; writes BOM-free UTF-8 via `File::WriteAllText`
- `Show-SSHConfigFile`:136 — full TUI pager with syntax colouring; uses buffer-mode `Show-OpBanner` at row 5; detects resize; Q exits
- `Edit-SSHConfigFile`:225 — respects `$env:EDITOR`; falls back through `code → nvim → vim → nano → notepad.exe`
- `Show-SSHKeyInventory`:252 — interactive table; Up/Dn navigates, Enter drills into `_ViewSSHKey`; uses buffer-mode `Show-OpBanner`; detects resize
- `_ViewSSHKey`:428 — submenu (public key / private key / back); shows red warning bar before displaying private key; Esc exits without viewing
- `_DisplayKeyFile`:470 — full-screen pager for raw key file content; Esc or Q closes
...
- **`Add-SSHKeyInHost` uses `Invoke-Expression`.** Built as a string to handle the empty-passphrase `-N ''` flag cleanly. Key name and comment must not contain shell-special characters (`"`, `` ` ``, `$`).
